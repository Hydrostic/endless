...
func NewServer(addr string, handler http.Handler) (srv *endlessServer) {
	...
	srv.BeforeBegin = func(addr string) {
		Logger.Info(syscall.Getpid(), addr)
	}
...
func (srv *endlessServer) Serve() (err error) {
	defer Logger.Info(syscall.Getpid(), "Serve() returning...")
	srv.setState(STATE_RUNNING)
	err = srv.Server.Serve(srv.EndlessListener)
	Logger.Info(syscall.Getpid(), "Waiting for connections to finish...")
	srv.wg.Wait()
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	Logger.Info(syscall.Getpid(), srv.Addr)
	return srv.Serve()
}
...
func (srv *endlessServer) handleSignals() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			Logger.Info(pid, "Received SIGHUP. forking.")
			err := srv.fork()
			if err != nil {
				Logger.Errorf("Fork err:", err)
			}
		case syscall.SIGUSR1:
			Logger.Info(pid, "Received SIGUSR1.")
		case syscall.SIGUSR2:
			Logger.Info(pid, "Received SIGUSR2.")
			srv.hammerTime(0 * time.Second)
		case syscall.SIGINT:
			Logger.Info(pid, "Received SIGINT.")
			srv.shutdown()
		case syscall.SIGTERM:
			Logger.Info(pid, "Received SIGTERM.")
			srv.shutdown()
		case syscall.SIGTSTP:
			Logger.Info(pid, "Received SIGTSTP.")
		default:
			Logger.Infof("Received %v: nothing i care about...\n", sig)
		}
...
func (srv *endlessServer) shutdown() {
	...
	if err != nil {
		Logger.Error(syscall.Getpid(), "Listener.Close() error:", err)
	} else {
		Logger.Info(syscall.Getpid(), srv.EndlessListener.Addr(), "Listener closed.")
	}
}
...
func (srv *endlessServer) hammerTime(d time.Duration) {
	...
	time.Sleep(d)
	Logger.Warn("[STOP - Hammer Time] Forcefully shutting down parent")
	for {
